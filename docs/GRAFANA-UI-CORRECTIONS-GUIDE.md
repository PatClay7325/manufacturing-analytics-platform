...
```css
/* src/styles/grafana-theme.css */
:root {
  /* Backgrounds */
  --grafana-bg-0: #0b0c0e;      /* Darkest */
  --grafana-bg-1: #111217;      /* Page background */
  --grafana-bg-2: #1f1f23;      /* Panel background */
  --grafana-bg-3: #26262b;      /* Hover states */
  
  /* Borders */
  --grafana-border-0: #000000;
  --grafana-border-1: #1f1f23;
  --grafana-border-2: #34343b;
  
  /* Text */
  --grafana-text: #d8d9da;
  --grafana-text-weak: #9fa0a2;
  --grafana-text-disabled: #6e7072;
  
  /* Primary colors */
  --grafana-primary: #3871dc;
  --grafana-primary-shade: #1f60c4;
  
  /* Status colors */
  --grafana-success: #1a7f4b;
  --grafana-warning: #ff9830;
  --grafana-error: #e02f44;
}
```
...
it('should match Analytics UI exactly', async () => {
  const grafanaScreenshot = await page.screenshot({ path: 'grafana-reference.png' });
  const ourScreenshot = await page.screenshot({ path: 'our-implementation.png' });
  
  const diff = await compareImages(grafanaScreenshot, ourScreenshot);
