...
export function VariableDropdown({ variable, onChange }: VariableDropdownProps) {
  ...
  return (
    <div className="grafana-variable-dropdown">
      {variable.hide !== 'label' && (
...
export function PanelEditor({ panel, onSave, onCancel }: PanelEditorProps) {
  ...
  return (
    <div className="grafana-panel-editor h-full flex flex-col bg-gray-900">
      {/* Header */}
...
```typescript
// src/__tests__/Analytics-engine/grafana-parity.test.ts
describe('Analytics Parity Tests', () => {
  ...
  describe('Import/Export', () => {
    it('should import Analytics Dashboard JSON', async () => {
      ...
      const dashboard = await importer.importDashboard(grafanaJson);
