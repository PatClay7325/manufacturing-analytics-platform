...
// Simple in-memory cache for manufacturing data (5 minute TTL)
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check cache first
    const cacheKey = `manufacturing-data-${Date.now() - (Date.now() % (2 * 60 * 1000))}`; // 2-minute cache buckets
    let context = getCachedData(cacheKey);
    
    if (!context) {
      // Get ALL relevant manufacturing data
      context = await getComprehensiveManufacturingData(lastUserMessage);
    ...
    const ollamaResponse = await fetch(`${OLLAMA_API_URL}/api/chat`, {
      ...
      body: JSON.stringify({
        ...
        options: {
          ...
          num_predict: 800, // Increased for better responses
          top_k: 40,
          top_p: 0.9,
...
async function getComprehensiveManufacturingData(query: string) {
  ...
  try {
    ...
    // Get limited work units with only latest metrics (OPTIMIZED)
    const workUnits = await prisma.workUnit.findMany({
      take: 10, // Limit to 10 most important units
      where: {
        status: { in: ['operational', 'maintenance'] } // Only active units
      },
      include: {
        WorkCenter: {
          include: {
        ...
        Alert: {
          where: { status: 'active' },
          take: 2, // Max 2 alerts per unit
        },
        PerformanceMetric: {
          orderBy: {
            timestamp: 'desc',
          },
          take: 1, // Only latest metric
        },
        Metric: {
          where: {
            timestamp: {
              gte: new Date(Date.now() - 4 * 60 * 60 * 1000), // Last 4 hours only
            },
          },
          orderBy: {
            timestamp: 'desc',
          },
          take: 5, // Max 5 recent metrics
        },
      },
    });

    // Get limited active alerts (OPTIMIZED)
    const activeAlerts = await prisma.alert.findMany({
      where: { status: 'active' },
      include: {
        WorkUnit: {
          select: { name: true } // Only need name
        },
      },
      orderBy: {
        severity: 'desc',
      },
      take: 5, // Limit to 5 most critical alerts
    });

    // Get recent production metrics summary (OPTIMIZED)
