...
const renameFolders = (dryRun: boolean) => {
  ...
  const grafanaDir = path.join(process.cwd(), 'src/components/grafana');
  ...
  const grafanaDashboardDir = path.join(process.cwd(), 'src/app/Analytics-dashboard');
  if (fs.existsSync(grafanaDashboardDir)) {
    const newDir = path.join(process.cwd(), 'src/app/Analytics-dashboard');
    if (dryRun) {
      console.log(`  Would rename: ${grafanaDashboardDir} → ${newDir}`);
    } else {
      fs.renameSync(grafanaDashboardDir, newDir);
      console.log(`✅ Renamed: ${grafanaDashboardDir} → ${newDir}`);
    }
  }
};
