...
async function seedComprehensiveData() {
  ...
  console.log('⚠️  IMPORTANT: This seeds REAL DATA ONLY - No mock data allowed!');
  ...
  try {
    ...
    if (sensorEventCount === 0) {
      ...
      const units = await prisma.dimUnit.findMany();
      if (units.length === 0) {
        console.log('  Creating units...');
        await prisma.dimUnit.createMany({
          data: [
            { code: 'TEMP_C', name: 'Celsius', type: 'Temperature' },
            { code: 'VIB_MMS', name: 'mm/s', type: 'Vibration' },
            { code: 'PRESS_BAR', name: 'bar', type: 'Pressure' },
            { code: 'CURR_A', name: 'Ampere', type: 'Current' },
            { code: 'SPEED_RPM', name: 'RPM', type: 'Speed' }
          ]
        });
      }
      
      const unitMap = await prisma.dimUnit.findMany().then(units => 
        units.reduce((acc, u) => ({ ...acc, [u.code]: u.id }), {})
      );
      
      const equipment = await prisma.dimEquipment.findMany({ take: 3 });
      const parameters = [
        { name: 'temperature', unitId: unitMap.TEMP_C, min: 60, max: 90 },
        { name: 'vibration', unitId: unitMap.VIB_MMS, min: 0.5, max: 5.0 },
        { name: 'pressure', unitId: unitMap.PRESS_BAR, min: 4, max: 6 },
        { name: 'current', unitId: unitMap.CURR_A, min: 10, max: 30 },
        { name: 'speed', unitId: unitMap.SPEED_RPM, min: 1000, max: 3000 }
      ];
    ...
    await prisma.dimDowntimeReason.updateMany({
      where: { 
        code: { in: ['MECH_FAIL', 'ELEC_FAIL', 'BREAKDOWN'] }
      },
      data: { isFailure: true }
    });
